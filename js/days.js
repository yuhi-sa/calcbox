document.addEventListener('DOMContentLoaded', function () {
  ...
  document.getElementById('calc-diff-btn').addEventListener('click', function () {
    ...
    document.getElementById('diff-days').textContent = days + '日間';
    document.getElementById('diff-weeks').textContent = weeks + '週' + remainDays + '日';
  ...
  document.getElementById('calc-add-btn').addEventListener('click', function () {
    ...
    var label = addDays >= 0
      ? toDateString(base) + 'から' + addDays + '日後'
      : toDateString(base) + 'から' + Math.abs(addDays) + '日前';
  ...
  document.getElementById('calc-countdown-btn').addEventListener('click', function () {
    ...
    if (days > 0) {
      ...
      countdownDays.textContent = '今日です！';
      countdownDays.style.color = '#3b82f6';
    } else {
      countdownDays.textContent = Math.abs(days) + ' 日前に過ぎています';
