document.addEventListener('DOMContentLoaded', function () {
  ...
  var ERAS = [
    { key: 'reiwa', name: '令和', startYear: 2019 },
    { key: 'heisei', name: '平成', startYear: 1989 },
    { key: 'showa', name: '昭和', startYear: 1926 },
    { key: 'taisho', name: '大正', startYear: 1912 },
    { key: 'meiji', name: '明治', startYear: 1868 }
  ];
  ...
  warekiToSeirekiBtn.addEventListener('click', function () {
    ...
    label.textContent = era.name + formatEraYear(eraYear);

    var value = document.createElement('span');
    value.className = 'result__value';
    value.textContent = '西暦 ' + westernYear + '年';
  ...
  seirekiToWarekiBtn.addEventListener('click', function () {
    ...
    for (var i = 0; i < ERAS.length; i++) {
      ...
      if (westernYear >= era.startYear) {
        ...
        var label = document.createElement('span');
        label.className = 'result__label';
        label.textContent = '西暦 ' + westernYear + '年';

        var value = document.createElement('span');
        value.className = 'result__value';
        value.textContent = era.name + formatEraYear(eraYear);
        ...
        // For years that span multiple eras (e.g. 2019 = both heisei 31 and reiwa 1)
        // Only show the next era if the year is the start year of the current era
    ...
    if (!found) {
      ...
      var label = document.createElement('span');
      label.className = 'result__label';
      label.textContent = '該当する元号が見つかりません。';
      item.appendChild(label);
      seirekiResultContent.appendChild(item);
    }
