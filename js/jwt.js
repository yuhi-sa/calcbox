document.addEventListener('DOMContentLoaded', function () {
  ...
  function decodeJWT() {
    ...
    if (parts.length !== 3) {
      errorMsg.textContent = 'JWTトークンの形式が正しくありません。xxxxx.yyyyy.zzzzz の3つのパートが必要です。';
    ...
    if (!headerStr || !payloadStr) {
      errorMsg.textContent = 'Base64urlデコードに失敗しました。トークンの形式を確認してください。';
      errorMsg.hidden = false;
      resultSection.hidden = true;
      return;
    }
    ...
    try {
      ...
      errorMsg.textContent = 'ヘッダーのJSONパースに失敗しました。';
      errorMsg.hidden = false;
      resultSection.hidden = true;
      return;
    }
    try {
      ...
      errorMsg.textContent = 'ペイロードのJSONパースに失敗しました。';
      errorMsg.hidden = false;
      resultSection.hidden = true;
      return;
    }
    ...
    if (typeof payload.exp === 'number') {
      ...
      if (payload.exp < now) {
        expStatus.textContent = '期限切れ（' + new Date(payload.exp * 1000).toLocaleString('ja-JP') + ' に失効）';
        ...
        expStatus.textContent = '有効（' + new Date(payload.exp * 1000).toLocaleString('ja-JP') + ' まで有効）';
        expStatus.style.background = '#dcfce7';
        expStatus.style.color = '#166534';
        expStatus.style.border = '1px solid #bbf7d0';
      }
    } else {
      expStatus.textContent = 'exp（有効期限）フィールドなし';
