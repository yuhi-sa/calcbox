document.addEventListener('DOMContentLoaded', function () {
  ...
  for (var i = 0; i < inputTypeRadios.length; i++) {
    inputTypeRadios[i].addEventListener('change', function () {
      ...
      if (type === 'annual') {
        salaryLabel.textContent = '額面年収（円）';
        salaryInput.placeholder = '例: 4000000';
      } else {
        salaryLabel.textContent = '額面月収（円）';
        salaryInput.placeholder = '例: 300000';
      }
    });
  }
  ...
  calcBtn.addEventListener('click', function () {
    ...
    if (isNaN(salaryVal) || salaryVal < 0) {
      alert('額面金額を正しく入力してください。');
      return;
    }
    if (isNaN(ageVal) || ageVal < 18 || ageVal > 100) {
      alert('年齢を正しく入力してください（18〜100）。');
      return;
    }
