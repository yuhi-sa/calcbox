...
var fallbackRates = {
USD: 1, JPY: 149.50, EUR: 0.92, GBP: 0.79, CNY: 7.24,
KRW: 1330, TWD: 32.5, AUD: 1.55, CAD: 1.36, CHF: 0.89
};
var ratesPerUSD = fallbackRates;
var ratesSource = 'fallback';
...
newRates[code] = data.rates[code] !== undefined ? data.rates[code] : fallbackRates[code];
}
...
if (toolBody) { toolBody.appendChild(noteEl); }
}
return noteEl;
}
...
var dateStr = ratesLastUpdated.getFullYear() + '/' + (ratesLastUpdated.getMonth() + 1) + '/' + ratesLastUpdated.getDate() + ' ' + h + ':' + m;
...
if (!amount || amount <= 0) { alert('正の金額を入力してください。'); return; }
if (from === to) { alert('異なる通貨を選択してください。'); return; }
