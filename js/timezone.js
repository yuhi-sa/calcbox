document.addEventListener('DOMContentLoaded', function () {
  ...
  // Default: source=Tokyo, target=New York
  sourceTzSelect.value = 'Asia/Tokyo';
  targetTzSelect.value = 'America/New_York';

  // Set default date/time to now in source tz
  ...
  function updateClocks() {
    ...
    clockElements.forEach(function (item) {
      var timeFmt = new Intl.DateTimeFormat('ja-JP', {
        timeZone: item.tz,
        hour: '2-digit', minute: '2-digit', second: '2-digit',
        hour12: false
      });
      var dateFmt = new Intl.DateTimeFormat('ja-JP', {
        timeZone: item.tz,
  ...
  convertBtn.addEventListener('click', function () {
    ...
    // Parse input as the source timezone
    // Create a date string and find the UTC offset for the source timezone
    var sourceTz = sourceTzSelect.value;
    var targetTz = targetTzSelect.value;

    // Build a Date object from the input
    // We use a trick: format a known date in the source tz to find the offset
    var inputDatetime = new Date(dateStr + 'T' + timeStr);
    if (isNaN(inputDatetime.getTime())) {
      alert('有効な日付と時刻を入力してください。');
      return;
    }

    // Get the local representation in source tz for the input date
    // We need to find the UTC time that corresponds to the given local time in source tz
    // Approach: use iterative correction
    ...
    // Get what the guess looks like in source tz
    ...
    // Format result in target timezone
    var resultFmt = new Intl.DateTimeFormat('ja-JP', {
      timeZone: targetTz,
      year: 'numeric', month: 'long', day: 'numeric',
      weekday: 'short',
      hour: '2-digit', minute: '2-digit', second: '2-digit',
