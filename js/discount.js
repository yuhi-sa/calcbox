document.addEventListener('DOMContentLoaded', function () {
  ...
  function toWaribiki(percent) {
    // Convert percent to Japanese "○割○分引き" display
    var wari = Math.floor(percent / 10);
    var bu = Math.round(percent % 10);
    if (bu === 0) {
      return wari + '割引き';
    }
    return wari + '割' + bu + '分引き';
  }
  ...
  calcBtn.addEventListener('click', function () {
    ...
    if (mode === 'percent') {
      ...
      if (isNaN(p1) || p1 < 0 || p1 > 100) {
        alert('1回目の割引率を0〜100の範囲で入力してください。');
        return;
      }
      if (isNaN(p2) || p2 < 0 || p2 > 100) {
        alert('2回目の割引率を0〜100の範囲で入力してください。');
        return;
      }
      ...
      // Show the effective combined rate
      var simpleSum = p1 + p2;
      resultDoubleDetail.textContent = formatPercent(Math.round(discountRate * 10) / 10) + '（単純合計 ' + formatPercent(simpleSum) + ' とは異なります）';
      resultDoubleDetailItem.hidden = false;
    }

    resultPrice.textContent = formatYen(discountedPrice);
    resultDiscount.textContent = formatYen(discountAmount);
    ...
    if (roundedRate >= 10 && roundedRate === Math.round(roundedRate)) {
      displayParts.push(toWaribiki(roundedRate));
    }
