document.addEventListener('DOMContentLoaded', function () {
  ...
  calcBtn.addEventListener('click', function () {
    ...
    if (isNaN(annualRate) || annualRate < 0 || annualRate > 50) {
      alert('年利を0〜50%の範囲で入力してください。');
      return;
    }
    if (isNaN(years) || years < 1 || years > 50) {
      alert('返済期間を1〜50年の範囲で入力してください。');
      return;
    }
    ...
    if (method === 'equal-payment') {
      ...
      monthlyLabel.textContent = '毎月の返済額';
      monthlyPaymentEl.textContent = formatYen(monthlyPayment);
      ...
      monthlyLabel.textContent = '初回の返済額（毎月変動）';
      monthlyPaymentEl.textContent = formatYen(firstMonthPayment) + ' 〜 ' + formatYen(lastMonthPayment);
    }

    totalPaymentEl.textContent = formatMan(totalPayment);
    totalInterestEl.textContent = formatMan(totalInterest);
