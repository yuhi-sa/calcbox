...
export const tools: ToolDefinition[] = [
  // --- 健康 ---
  { id: 'bmi', name: 'BMI計算・適正体重チェック', description: '身長と体重からBMI値を計算し、肥満度や適正体重を確認できます。', category: 'health', icon: 'user', implemented: true, keywords: ['taijuu', 'shinchou', '体重', '身長', '肥満'] },
  { id: 'body-fat', name: '体脂肪率計算', description: '身長・体重・年齢・性別から体脂肪率を推定。判定表示付き。', category: 'health', icon: 'user-plus', implemented: true, keywords: ['taishibo', 'diet'] },
  { id: 'calorie', name: '消費カロリー計算', description: '運動や日常活動で消費するカロリーを計算します。ダイエットの参考に。', category: 'health', icon: 'flame', implemented: true, keywords: ['undou', 'diet', 'ダイエット', '運動'] },
  { id: 'pet-age', name: 'ペット年齢換算', description: '犬・猫の年齢を人間の年齢に換算。サイズ別の換算にも対応。', category: 'health', icon: 'paw', implemented: true, keywords: ['inu', 'neko', '犬', '猫'] },
  { id: 'pregnancy', name: '妊娠週数・出産予定日計算', description: '最終月経日から妊娠週数と出産予定日を自動計算。', category: 'health', icon: 'clock', implemented: true, keywords: ['ninshin', 'shussan', '出産'] },
  { id: 'sleep', name: '睡眠時間計算', description: '睡眠サイクル（90分周期）に基づく最適な就寝・起床時間を計算。', category: 'health', icon: 'moon', implemented: true, keywords: ['suimin', '起床', '就寝'] },
  { id: 'bmr', name: '基礎代謝（BMR）計算', description: '年齢・身長・体重・性別から基礎代謝量と1日の消費カロリーを計算。', category: 'health', icon: 'flame', implemented: true, keywords: ['kisotaisha'] },
  { id: 'alcohol', name: 'アルコール分解時間計算', description: '飲酒量・体重からアルコール分解時間と運転可能時刻の目安を計算。', category: 'health', icon: 'wine', implemented: true, keywords: ['inshu', 'sake', '酒', '飲酒', '運転'] },
  { id: 'ideal-weight', name: '理想体重・ダイエット計算', description: '身長から理想体重を算出し、目標達成に必要なカロリー制限を提案。', category: 'health', icon: 'scale', implemented: true, keywords: ['risou', 'diet'] },
  { id: 'nutrition', name: '栄養素・カロリー計算', description: '食品の栄養素とカロリーを計算。38種類の食品データベース搭載。', category: 'health', icon: 'cup', implemented: true, keywords: ['eiyou', '栄養', '食品'] },

  // --- 生活 ---
  { id: 'tax', name: '消費税計算（税込・税抜）', description: '税込価格と税抜価格を瞬時に変換。軽減税率8%にも対応。', category: 'life', icon: 'table', implemented: true, keywords: ['shouhizei', 'zeikomi', 'zeinuki', '税金'] },
  { id: 'warikan', name: '割り勘計算', description: '飲み会やお食事の会計をかんたんに割り勘。端数処理もおまかせ。', category: 'life', icon: 'dollar', implemented: true, keywords: ['nomikai', '飲み会', 'split'] },
  { id: 'electricity', name: '電気代計算', description: '家電の消費電力から月間・年間の電気代を計算。節電の参考に。', category: 'life', icon: 'zap', implemented: true, keywords: ['denkidai', '電力', 'watt'] },
  { id: 'unit', name: '単位変換', description: '長さ・重さ・温度・面積の単位を簡単に変換。坪↔平米にも対応。', category: 'life', icon: 'arrows', implemented: true, keywords: ['tani', 'henkan', 'cm', 'kg', '坪', '平米'] },
  { id: 'discount', name: '割引計算', description: '割引率や割引額から割引後の価格を計算。二重割引にも対応。', category: 'life', icon: 'percent', implemented: true, keywords: ['waribiki', 'sale', 'セール', '% off'] },
  { id: 'area', name: '面積計算', description: '各種図形の面積を計算。坪・畳・ヘクタールへの換算にも対応。', category: 'life', icon: 'grid', implemented: true, keywords: ['menseki', '坪', '畳'] },
  { id: 'speed', name: '速度・距離・時間計算', description: '速度・距離・時間の関係を計算。マラソンペース計算にも対応。', category: 'life', icon: 'layers', implemented: true, keywords: ['sokudo', 'kyori', 'marathon'] },
  { id: 'random', name: '乱数生成器', description: '指定範囲のランダムな数値を生成。抽選やくじ引きに便利。', category: 'life', icon: 'dice', implemented: true, keywords: ['ransuu', 'kuji', 'くじ', '抽選'] },
  { id: 'currency', name: '通貨換算', description: '主要10通貨間のレート換算。JPY・USD・EUR・GBPなど対応。', category: 'life', icon: 'dollar', implemented: true, keywords: ['tsuka', 'kawase', 'yen', 'dollar', 'euro', '為替'] },
  ...
  // --- 開発者 ---
  { id: 'password', name: 'パスワード生成', description: '安全なパスワードを自動生成。強度メーター付き。', category: 'developer', icon: 'lock', implemented: true, keywords: ['password', 'セキュリティ'] },
  { id: 'hash', name: 'ハッシュ生成', description: 'テキストからSHA-1/SHA-256/SHA-512ハッシュを生成。', category: 'developer', icon: 'shield', implemented: true, keywords: ['sha', 'md5', 'hash'] },
  { id: 'uuid', name: 'UUID生成', description: 'v4 UUIDを生成。一括生成・バリデーション機能付き。', category: 'developer', icon: 'hash', implemented: true, keywords: ['guid', 'uuid'] },
  { id: 'base-converter', name: '基数変換', description: '2進数・8進数・10進数・16進数を相互変換。', category: 'developer', icon: 'hash', implemented: true, keywords: ['binary', 'hex', 'octal', '2進数', '16進数'] },
  { id: 'color-converter', name: 'カラーコード変換', description: 'HEX・RGB・HSLのカラーコードを相互変換。', category: 'developer', icon: 'grid', implemented: true, keywords: ['color', 'hex', 'rgb', 'hsl', '色'] },
  { id: 'json-formatter', name: 'JSONフォーマッター', description: 'JSONの整形・圧縮・バリデーションツール。', category: 'developer', icon: 'code', implemented: true, keywords: ['json', 'format', 'lint'] },
  { id: 'encode', name: 'エンコード・デコード', description: 'Base64・URL・HTMLエンティティのエンコード/デコード。', category: 'developer', icon: 'code', implemented: true, keywords: ['base64', 'url', 'encode', 'decode'] },
  { id: 'regex', name: '正規表現テスター', description: '正規表現パターンをリアルタイムテスト。マッチ箇所をハイライト。', category: 'developer', icon: 'search', implemented: true, keywords: ['regex', 'seiki', '正規表現'] },
  { id: 'ip-calc', name: 'IPアドレス・サブネット計算', description: 'CIDRからネットワークアドレス・ブロードキャスト・ホスト範囲を計算。', category: 'developer', icon: 'globe', implemented: true, keywords: ['ip', 'subnet', 'cidr', 'network'] },
  { id: 'cron', name: 'Cron式パーサー', description: 'Cron式を解析し次回実行時刻を表示。', category: 'developer', icon: 'clock', implemented: true, keywords: ['cron', 'crontab', 'schedule'] },
  { id: 'chmod', name: 'chmod計算機', description: 'ファイルパーミッションをチェックボックスで直感的に計算。', category: 'developer', icon: 'lock', implemented: true, keywords: ['chmod', 'permission', 'linux', 'unix'] },
  { id: 'jwt', name: 'JWTデコーダー', description: 'JWTトークンのヘッダーとペイロードをデコード。有効期限チェック付き。', category: 'developer', icon: 'shield', implemented: true, keywords: ['jwt', 'token', 'auth'] },
  { id: 'csv-json', name: 'CSV⇔JSON変換', description: 'CSVとJSONの双方向変換ツール。', category: 'developer', icon: 'file-text', implemented: true, keywords: ['csv', 'json', 'convert'] },
  { id: 'sql-formatter', name: 'SQLフォーマッター', description: 'SQLクエリを自動整形して見やすく。', category: 'developer', icon: 'code', implemented: true, keywords: ['sql', 'query', 'format'] },
  { id: 'byte-converter', name: 'バイト変換', description: 'B/KB/MB/GB/TB/PBの相互変換。', category: 'developer', icon: 'arrows', implemented: true, keywords: ['byte', 'kb', 'mb', 'gb', 'tb'] },
  { id: 'http-status', name: 'HTTPステータスコード一覧', description: 'HTTPステータスコードの意味と説明を一覧表示。', category: 'developer', icon: 'globe', implemented: true, keywords: ['http', 'status', '200', '404', '500'] },
  { id: 'markdown', name: 'Markdownプレビュー', description: 'Markdownをリアルタイムプレビュー。HTML出力コピー機能付き。', category: 'developer', icon: 'file-text', implemented: true, keywords: ['markdown', 'md', 'preview'] },
  { id: 'gradient', name: 'CSSグラデーション生成', description: 'リニア・ラジアルCSSグラデーションをビジュアルに生成。', category: 'developer', icon: 'grid', implemented: true, keywords: ['css', 'gradient', 'color'] },
  { id: 'qr-generator', name: 'QRコード生成', description: 'テキストやURLからQRコードを即座に生成。', category: 'developer', icon: 'grid', implemented: true, keywords: ['qr', 'barcode', 'url'] },
  { id: 'lorem', name: 'ダミーテキスト生成', description: '段落・文・単語数を指定してLorem Ipsumダミーテキストを生成。', category: 'developer', icon: 'file-text', implemented: true, keywords: ['lorem', 'ipsum', 'dummy', 'placeholder'] },
];
