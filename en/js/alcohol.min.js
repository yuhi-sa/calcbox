document.addEventListener('DOMContentLoaded', function () {
  ...
  function addHoursToTime(timeStr, addHours) {
    ...
    if (days > 0) {
      timeFormatted += ' (next day';
      if (days > 1) timeFormatted += ' +' + days;
      timeFormatted += ')';
    }
  ...
  calcBtn.addEventListener('click', function () {
    ...
    if (isNaN(weight) || weight < 30 || weight > 200) {
      alert('Please enter a valid weight (30-200kg).');
      return;
    }
    if (!drinkTime) {
      alert('Please enter the drinking start time.');
      return;
    }
    ...
    if (totalAlcohol <= 0) {
      alert('Please enter the amount of alcohol consumed.');
      return;
    }
    ...
    resultDecomposeHours.textContent = hours + ' hours ' + minutes + ' minutes';
