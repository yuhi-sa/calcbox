document.addEventListener('DOMContentLoaded', function () {
  ...
  calcBtn.addEventListener('click', function () {
    ...
    if (isNaN(years) || years < 1 || years > 100) {
      alert('Please enter an investment period between 1 and 100 years.');
      return;
    }
    if (principal <= 0 && monthly <= 0) {
      alert('Please enter a principal amount or monthly contribution.');
      return;
    }
    ...
    for (var year = 1; year <= years; year++) {
      ...
      tr.innerHTML =
        '<td style="padding: 6px 8px; text-align: right; border-bottom: 1px solid var(--color-border);">Year ' + year + '</td>' +
        '<td style="padding: 6px 8px; text-align: right; border-bottom: 1px solid var(--color-border);">' + formatCurrency(totalInvested) + '</td>' +
        '<td style="padding: 6px 8px; text-align: right; border-bottom: 1px solid var(--color-border);">' + formatCurrency(interest) + '</td>' +
        '<td style="padding: 6px 8px; text-align: right; border-bottom: 1px solid var(--color-border);">' + formatCurrency(balance) + '</td>';
    ...
    resultTotal.textContent = formatCurrency(balance);
    resultInvested.textContent = formatCurrency(finalInvested);
