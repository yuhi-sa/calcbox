document.addEventListener('DOMContentLoaded', function () {
  ...
  function getCategory(bmi) {
    ...
    if (bmi < 30) return { label: 'Obese (Class I)', color: '#f59e0b' };
    if (bmi < 35) return { label: 'Obese (Class II)', color: '#f97316' };
    if (bmi < 40) return { label: 'Obese (Class III)', color: '#ef4444' };
    return { label: 'Obese (Class IV)', color: '#dc2626' };
  }
  ...
  calcBtn.addEventListener('click', function () {
    ...
    if (!h || !w || h < 100 || h > 250 || w < 20 || w > 300) {
      alert('Please enter valid height (100-250cm) and weight (20-300kg).');
      return;
    }
    ...
    if (diff > 0) {
      weightDiff.textContent = '+' + diff.toFixed(1) + ' kg (above ideal weight)';
      weightDiff.style.color = '#f59e0b';
    } else if (diff < 0) {
      weightDiff.textContent = diff.toFixed(1) + ' kg (below ideal weight)';
      weightDiff.style.color = '#3b82f6';
    } else {
      weightDiff.textContent = '0 kg (at ideal weight)';
