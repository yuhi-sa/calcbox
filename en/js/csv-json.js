document.addEventListener('DOMContentLoaded', function () {
  ...
  csvToJsonBtn.addEventListener('click', function () {
    ...
    try {
      ...
      resultInfo.textContent = 'CSV → JSON | ' + data.length + ' records';
      resultSection.hidden = false;
      showPreview(data, hasHeader);
      resultSection.scrollIntoView({ behavior: 'smooth' });
    } catch (e) {
      showError('CSV conversion error: ' + e.message);
    }
  });
  ...
  jsonToCsvBtn.addEventListener('click', function () {
    ...
    try {
      ...
      if (!Array.isArray(data)) {
        showError('Please enter a JSON array (e.g. [{...}, {...}]).');
        return;
      }
      var csvStr = jsonToCsv(data, delimiter);
      csvInput.value = csvStr;
      output.textContent = csvStr;
      resultInfo.textContent = 'JSON → CSV | ' + data.length + ' records';
      resultSection.hidden = false;
